...
const { WAFRegionalClient, ListWebACLsCommand: ListWebACLsRegionalCommand, AssociateWebACLCommand: AssociateWebACLRegionalCommand, GetWebACLForResourceCommand: GetWebACLForResourceRegionalCommand, DisassociateWebACLCommand: DisassociateWebACLRegionalCommand } = require('@aws-sdk/client-waf-regional')
const { WAFV2Client, ListWebACLsCommand: ListWebACLsV2Command, AssociateWebACLCommand: AssociateWebACLV2Command, GetWebACLForResourceCommand: GetWebACLForResourceV2Command, DisassociateWebACLCommand: DisassociateWebACLV2Command } = require('@aws-sdk/client-wafv2')
...
class AssociateWafPlugin {
  ...
  async getCloudFormationClient() {
    if (!this.cloudFormationClient) {
      this.cloudFormationClient = new CloudFormationClient(await this.provider.getAwsSdkV3Config())
    }
  ...
  async getWafClient() {
    if (!this.wafClient) {
      ...
      this.wafClient = this.wafVersion === DEFAULT_WAF_VERSION ? new WAFRegionalClient(config) : new WAFV2Client(config)
    }
  ...
  async findWebAclByName(name) {
    ...
    const client = await this.getWafClient()
    const ListWebACLsCommand = this.wafVersion === DEFAULT_WAF_VERSION ? ListWebACLsRegionalCommand : ListWebACLsV2Command
    const response = await client.send(new ListWebACLsCommand(params))
    if (response.WebACLs) {
  ...
  async findStackResourceByLogicalId(stackName, logicalId) {
    const client = await this.getCloudFormationClient()
  ...
  async associateWaf() {
    try {
      ...
      const client = await this.getWafClient()
      const AssociateWebACLCommand = this.wafVersion === DEFAULT_WAF_VERSION ? AssociateWebACLRegionalCommand : AssociateWebACLV2Command
      await client.send(new AssociateWebACLCommand(params))
    } catch (e) {
      this.log.error(`\n-------- Associate WAF Error --------\n${e.message}`)
    }
  }
  ...
  async disassociateWaf() {
    try {
      ...
      const client = await this.getWafClient()
      const GetWebACLForResourceCommand = this.wafVersion === DEFAULT_WAF_VERSION ? GetWebACLForResourceRegionalCommand : GetWebACLForResourceV2Command
      const webACLForResource = await client.send(new GetWebACLForResourceCommand(params))
      if (webACLForResource.WebACLSummary || webACLForResource.WebACL) { //WAFV2 uses WebACL
        this.log.info('Disassociating WAF...')
        const DisassociateWebACLCommand = this.wafVersion === DEFAULT_WAF_VERSION ? DisassociateWebACLRegionalCommand : DisassociateWebACLV2Command
        await client.send(new DisassociateWebACLCommand(params))
      }
